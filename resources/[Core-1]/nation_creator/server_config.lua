...
if multiCharacter then
    vRP._prepare("nation_creator/createAgeColumn","ALTER TABLE summerz_characters ADD IF NOT EXISTS age INT(11) NOT NULL DEFAULT 20")
    vRP._prepare("nation_creator/update_user_first_spawn","UPDATE summerz_characters SET name2 = @name2, name = @name, age = @age, phone = @phone, serial = @serial, blood = @blood WHERE id = @user_id")
    vRP._prepare("nation_creator/create_characters","INSERT INTO summerz_characters(steam) VALUES(@steam)")
    vRP._prepare("nation_creator/remove_characters","UPDATE summerz_characters SET deleted = 1 WHERE id = @id")
    vRP._prepare("nation_creator/get_characters","SELECT * FROM summerz_characters WHERE steam = @steam and deleted = 0")
    vRP._prepare("nation_creator/get_character","SELECT * FROM summerz_characters WHERE steam = @steam and deleted = 0 and id = @user_id")
    vRP._prepare("nation_creator/get_charinfo","SELECT * FROM summerz_characters WHERE id = @user_id")
    CreateThread(function() vRP.execute("nation_creator/createAgeColumn") end) -- criar coluna idade na db
else
    vRP._prepare("nation_creator/update_user_first_spawn","UPDATE summerz_characters SET firstname = @firstname, name = @name, age = @age WHERE user_id = @user_id")
end
...
RegisterCommand('spawn',function(source) -- COMANDO DE ADMIN PARA SIMULAR O SPAWN
    if func.checkPermission({"manager.permissao", "mod.permissao", "Admin"}, source) or not vRP.getUserId(source) then
        if multiCharacter then
