...
local dismantleVehs = {
	...
	"taco","glendale","intruder","ruffian","schafter2","asterope","mixer2","rumpo","exemplar","surfer",
}
-----------------------------------------------------------------------------------------------------------------------------------------
-- INVENTORY:TABLES
-----------------------------------------------------------------------------------------------------------------------------------------
...
local stealItens = {
	...
	[33] = { ["item"] = "tablet", ["min"] = 1, ["max"] = 1, ["rand"] = 75 },
	...
	[42] = { ["item"] = "cellphone", ["min"] = 1, ["max"] = 1, ["rand"] = 250 }
}
-----------------------------------------------------------------------------------------------------------------------------------------
-- LOOTITENS
-----------------------------------------------------------------------------------------------------------------------------------------
...
AddEventHandler("inventory:useItem",function(Slot,Amount)
	...
	if user_id and Active[user_id] == nil then
		...
		if nameItem == "attachsFlashlight" or nameItem == "attachsCrosshair" or nameItem == "attachsMagazine" or nameItem == "attachsSilencer" or nameItem == "attachsGrip" then
			local returnWeapon = vCLIENT.returnWeapon(source)
			if returnWeapon then
			...
			if string.sub(nameItem,1,5) == "badge" then
				if openIdentity[user_id] then
					...
					if identity then
						...
						TriggerClientEvent("vRP:Identity",source,{ mode = "fidentity", nome = identity["name"].." "..identity["name2"],passaporte = user_id, nacionalidade = identity["locate"], porte = identity["port"], sangue = bloodTypes(identity["blood"]) })
					end
				end
			return end
				...
				if openIdentity[user_id] then
					...
					if identity then
						...
						if numberIdentity == user_id then
                            ...
                            TriggerClientEvent("vRP:Identity",source,{ mode = "identity", nome = identity["name"].." "..identity["name2"],passaporte = user_id, nacionalidade = identity["locate"], veiculos = identity["garage"], gemas = vRP.userGemstone(identity["steam"]), porte = identity["port"], premium = premium, sangue = bloodTypes(identity["blood"]) })
                        else
                            TriggerClientEvent("vRP:Identity",source,{ mode = "fidentity", nome = identity["name"].." "..identity["name2"],passaporte = user_id, nacionalidade = identity["locate"], porte = identity["port"], sangue = bloodTypes(identity["blood"]) })
                        end
                    end
				...
				if otherPlayer then
					if vPLAYER.getHandcuff(otherPlayer) then
						...
						if nuser_id then
							...
							if vRP.hasGroup(nuser_id,"Policia") then
								if vRP.tryGetInventoryItem(user_id,totalName,1,true,Slot) then
									vRP.removePermission(nuser_id,"Police")
									vRP.updatePermission(nuser_id,"Policia","waitPolicia")
									TriggerEvent("blipsystem:serviceExit",otherPlayer)
									TriggerClientEvent("radio:outServers",otherPlayer)
									TriggerClientEvent("vRP:PoliceService",otherPlayer,false)
									TriggerClientEvent("service:Label",otherPlayer,"Policia","Entrar em Serviço",5000)
									TriggerClientEvent("Notify",source,"amarelo","Todas as comunicações foram retiradas.",5000)
								end
							end
