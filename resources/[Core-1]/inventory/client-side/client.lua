...
RegisterNUICallback("sendItem",function(data)
	if MumbleIsConnected() then
		if data.amount > 0 then
			...
			TriggerEvent("Notify","vermelho","Não é possível pegar um valor negativo.",10000)
		end
	end
end)
-----------------------------------------------------------------------------------------------------------------------------------------
...
Citizen.CreateThread(function()
	while true do
		...
		for _,v in pairs(Drops) do
			...
			if distance <= 50 then
				timeDistance = 1
				DrawMarker(21,v["coords"][1],v["coords"][2],v["coords"][3] + 0.25,0.0,0.0,0.0,0.0,180.0,0.0,0.25,0.35,0.25,40,137,182,100,0,0,0,1)
			end
		end
