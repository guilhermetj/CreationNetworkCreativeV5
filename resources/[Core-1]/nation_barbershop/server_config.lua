...
function setPlayerTattoos(source, user_id)
    local data = vRP.userData(user_id, "Tattoos")
    if next(data) ~= nil then
        tattoos = data or {}
...
-- DEBUG
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterServerEvent("nation_barbershop:debug")
AddEventHandler("nation_barbershop:debug",function()
	local source = source
	local user_id = vRP.getUserId(source)
	if user_id then
		TriggerClientEvent("nation_barbershop:apply",source,vRP.userData(user_id,"Barbershop"))
		TriggerClientEvent("skinshop:apply",source,vRP.userData(user_id,"Clothings"))
		TriggerClientEvent("nation_tattoos:apply",source,vRP.userData(user_id,"Tatuagens"))
