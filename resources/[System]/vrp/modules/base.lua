...
-- DISCORDHOOK
-----------------------------------------------------------------------------------------------------------------------------------------
local webHook = "https://discord.com/api/webhooks/1083520478121033738/KTjOhVbI_QWiWUSd5T_bg8ggdnjQKFDdWPxBUcSKWF5HOnpfPoOaFqs3FTQ0dc9QxzrG"
-----------------------------------------------------------------------------------------------------------------------------------------
-- MYSQL
-----------------------------------------------------------------------------------------------------------------------------------------
local mysqlDriver
...
function playerDropped(source,reason)
	...
	if user_id then
		print("Saving " .. user_id .. " Reason: " .. reason)
		TriggerEvent("steamLogs","Disconnect","**Source:** "..parseFormat(source).."\n**Passaporte:** "..parseFormat(user_id).."\n**Motivo:** "..reason.."\n**Horário:** "..os.date("%H:%M:%S"),3092790)
		...
		if not vRP.checkBanned(steam) then
			...
			if infoAccount then
				if infoAccount["whitelist"] then
					...
					deferrals.done("Seu passaporte não esta autorizado! Entre no nosso steam: http://steam.gg/CONVITE/ para mais informações, Passaporte: "..steam)
				end
			else
			...
			deferrals.done("Você está banido da cidade.")
		end
	else
		deferrals.done("Ocorreu um problema ao conectar com seu steam.")
	end
...
function vRP.characterChosen(source,user_id,model,locate)
	...
	if userBank then
		vRP.userInfos[user_id]["bank"] = userBank["bank"]
	end
	...
	if infoAccount then
		...
		PerformHttpRequest(webHook,function(err,text,headers) end,"POST",json.encode({
			content = infoAccount["steam"].." #"..user_id.." "..identity["name"]
...
AddEventHandler("vRP:Print",function(message)
	...
	local user_id = vRP.getUserId(source)
	if user_id then
		TriggerEvent("steamLogs","Hackers","Passaporte **"..user_id.."** "..message..".",3092790)
	end
end)
-----------------------------------------------------------------------------------------------------------------------------------------
