...
Citizen.CreateThread(function()
	TriggerServerEvent("spotify:GetDate")
end)
...
RegisterNUICallback("action", function(data)
	...
	elseif data.action == "play" then
		if xSound:soundExists(nameid) then
			if xSound:isPaused(nameid) then
				TriggerServerEvent("spotify:ChangeState", true, nameid)
				local esperar = 0
				while nuiaberto do
					Wait(1000)
	...
	elseif data.action == "pause" then
		if xSound:soundExists(nameid) then
			if xSound:isPlaying(nameid) then
				TriggerServerEvent("spotify:ChangeState", false, nameid)
			end
		end
	...
	elseif data.action == "loop" then
		if xSound:soundExists(nameid) then
			...
			TriggerServerEvent("spotify:ChangeLoop",nameid,datasoundinfo.loop)
		else
	...
	elseif data.action == "forward" then
		if xSound:soundExists(nameid) then
			TriggerServerEvent("spotify:ChangePosition", 10, nameid)
		end
...
function ApplySound(quanti,plate)
	...
	if vadi <= 1.01 and vadi >= -0.001 and exis then
		...
		TriggerServerEvent("spotify:ChangeVolume", quanti, plate)
	end
end
...
function SetUrl(url,nid)
	...
	if url then
		...
		if encontrad then
			...
			TriggerServerEvent("spotify:ModifyURL",vehdata)
		else
			if IsPedInAnyVehicle(PlayerPedId(), false) then
				local veh = GetVehiclePedIsIn(PlayerPedId(),false)
				...
				vehdata.loop = datasoundinfo.loop
				TriggerServerEvent("spotify:AddVehicle",vehdata)
			end
		end
	else
...
RegisterNetEvent("spotify:ShowNui")
	AddEventHandler("spotify:ShowNui", function()
	show()
end)
...
AddEventHandler("spotify:AddVehicle", function(data)
	...
	if not cfg.PlayToEveryone and v.popo then
		avancartodos = 0.0
...
AddEventHandler("spotify:ChangeState", function(tipo, nome)
...
AddEventHandler("spotify:ChangeLoop", function(tipo, nome)
	if xSound:soundExists(nome) then
...
AddEventHandler("spotify:ChangeVolume", function(som, range, nome)
	local carroe
	local crds
    for i = 1, #Zones do
        local v = Zones[i]
...
RegisterNetEvent("spotify:SendData")
AddEventHandler("spotify:SendData", function(data)
    Zones = data
    for i = 1, #Zones do
		local v = Zones[i]
		if v.isplaying then
