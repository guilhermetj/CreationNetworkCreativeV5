...
RegisterCommand("wall",function()
	...
	if showIds then
		...
		TriggerEvent("Notify","importante","Sistema ativado.",5000)
	else
		vSERVER.reportLog("OFF")
		TriggerEvent("Notify","importante","Sistema desativado.",5000)
	end
end)
-----------------------------------------------------------------------------------------------------------------------------------------
-- GLOBALTHREAD
					...
					if admin then
						if ((distance < wallDistance)) then
							if GetPlayerPed(id) ~= -1 and players[id] ~= nil then
								...
								local playerHealthPercent = playerHealth
