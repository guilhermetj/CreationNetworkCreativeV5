...
function cRP.checkSystems()
	...
	if user_id then
		...
		if parseInt(#policeResult) <= 0 or os.time() < atmTimers then
			...
			if vRP.tryGetInventoryItem(user_id,"floppy",1,true) then
				vRP.upgradeStress(user_id,10)

				atmTimers = os.time() + 180
...
function cRP.paymentSystems(locate)
	...
	if user_id then
		if os.time() < atmTimers then
			local randPrice = math.random(120,190)

			TriggerEvent("Wanted",source,user_id,7)
			vRP.generateItem(user_id,"dollars2",parseInt(randPrice),true)

			local identity = vRP.userIdentity(user_id)
			if identity["locate"] ~= locate then
				vRP.generateItem(user_id,"dollars2",parseInt(randPrice * 0.6),true)
			end
		end
	end
end